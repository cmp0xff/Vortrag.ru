...
constant\footfullcite{COPELAND2006,Bamba2012}, but its origin has not yet been 
understood.
\item Proposals have been made of quintessence and phantom fields, which are 
minimally-coupled real scalar fields with $\mscrl = \pm 1$\footnote{The 
signature of metric is mostly positive.}
\begin{align}
\mscrS = \int\dd^4 x\,\sqrt{-g}\,\cbr{-\mscrl \frac{g^{\mu\nu}}{2}
\rbr{\partial_\mu\phi}\rbr{\partial_\nu\phi} - \rfun{\mscrV}{\phi}}.
\end{align}

\item $\rfun{\mscrV}{\phi} = V\ee^{\lambda\phi}$ where $\lambda, V\in \BbbR$ is 
of interest, and can be called Liouville\footfullcite{NAKAYAMA2004}.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Assuming homogeneity and isotropy, one applies a flat Robertson--Walker 
metric
\begin{align}
\dd s^2 = -\rfun{N^2}{t}\,\dd t^2 + \varkappa 
\ee^{2\rfun{\alpha}{t}}\,\dd\Omega_{3\text{F}}^2
\end{align}
with $\varkappa \coloneqq 8\pp\nG$, $\dd\Omega_{3\text{F}}^2 \coloneqq 
\dd\chi^2+\chi^2\rbr{\dd\theta^2+\sin^2\theta\,\dd\varphi^2}$.
...
quantum-mechanically, enabling one studying the relation between the classical
and quantum theory.
\end{itemize}
\end{frame}
...
% and the result is automatically consistent with the trsfed.\ Klein--Gordon 
% equation.
...
% } = 0,
\rfun{\ddot{\varphi}}{\beta} &\coloneqq
...
%{233}
Levels of the phantom model are discretised if one imposes Hermiticity of 
squared momenta under the Schrödinger product.
...
\item Wave packets with multiple branches in $\rbr{+,-}$ and $\rbr{-,+}$ is to
be understood
...
\begin{itemize}
\item Non-Hermitian, $PT$-symmetric Liouville 
fields\footfullcite{ANDRIANOV2010,Andrianov2016}: may cross the phantom divide
\end{itemize}
