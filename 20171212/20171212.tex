...
\item Field in which $\rfun{\mscrV}{\phi} = V\ee^{\lambda\phi}$, 
$\lambda, V\in \BbbR$ is of interest, and can be called 
Liouville field\footfullcite{NAKAYAMA2004}.
\end{itemize}
\end{frame}
...
quantum-mechanically, enabling one to study the relation between the classical
and quantum theory.
\end{itemize}
\end{frame}
...
\footfullcite[The same first integral has been found in][in canonical 
formalism]{Lan2016,Andrianov2015}
\begin{align}
...
\item Using the classical sinularities as boundary condition, one can fix 
$\nu_0 = 1$.
\end{itemize}
