...
\begin{frame}%
{Introduction}%
{Friedmann--Lemaître model with quintessence and phantom Liouville fields}
\begin{itemize}
...
\subsection{Lagrangian formalism and a first integral}
...
\section{The Dirac quantisation and the self-adjointness of Hamiltonian}

\subsection{The Dirac quantisation and the quantum-mechanical analogy}
...
\begin{itemize}
\item Fourier-transforming $\beta$ to $k_\beta$ yields
\begin{align}
\mscrl E \rfun{\psi}{x} = \widehat{H}_\text{eff} \rfun{\psi}{x} \coloneqq
\rbr{-\frac{\hslash^2}{2 \mitsansM_\text{P}} \partial_x^2
+ \mscrl \mscrs \mscrv \widetilde{V} \ee^{\widetilde{g} x}} \rfun{\psi}{x},
\end{align}
in which $\mitsansM_\text{P} \coloneqq \varkappa^{-1/2}$,
$x \coloneqq \varkappa \chi$,
$\widetilde{V} \coloneqq \varkappa^{3/2} \vbr{V} \ge 0$, and
$E \coloneqq \hslash^2 \mitsansM_\text{P} / 12 \cdot k_\beta^2 \ge 0$.
%, and $\widetilde{g} \coloneqq \varkappa^{-1} g$.

\item For the quintessence model $\rbr{+, +}$:
$\widehat{H}_\text{eff}$ bounded below; generalised eigenfunctions are
the Besselian $\BesselK_{\ii E}$'s.

\item For the quintessence model  $\rbr{+, -}$:
$\widehat{H}_\text{eff}$ bounded \alert{above}; generalised eigenfunctions 
for positive spectrum / `scattering states' are the Besselian
$\BesselF_{\ii E}$'s and $\BesselG_{\ii E}$'s\footfullcite{Dunster1990}.

\item For the phantom model $\rbr{-, +}$:
$\widehat{H}_\text{eff}$ bounded \alert{above}; generalised eigenfunctions 
for negative spectrum / `bound states' are the Besselian $\BesselJ_{E}$'s.
\end{itemize}
...
\rfun{\sech}{\gamma}$, which matches the trajectory with
$\beta_0 = 0$ if
\begin{align}
