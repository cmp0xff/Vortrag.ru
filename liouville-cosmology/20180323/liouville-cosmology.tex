...
conserved\footfullcite[The same first integral has been found in][in canonical 
...
%\ee^{g\chi} &= p_\beta^2/12\varkappa^2\vbr{V}\cdot
...
special solution\footfullcite[For instance,][ch.\ 3]{Liddle2000}.
...
time-independent Schr\"odinger equation for $\widehat{H}_\text{eff}$
\begin{align}
...
+ \mscrl \mscrs \mscrv \widetilde{V} \ee^{\widetilde{g} x}} \rfun{\psi}{x},
\end{align}
...
$\widehat{H}_\text{eff}$ is bounded \textcolor{blue}{below}; generalised
eigenfunctions are the Besselian $\BesselK_{\ii \nu}$'s.

\item For the quintessence model  $\rbr{+, -}$:
$\widehat{H}_\text{eff}$ is bounded \alert{above}; generalised eigenfunctions 
for positive spectrum / `scattering states' are the Besselian
$\BesselF_{\ii \nu}$'s and $\BesselG_{\ii \nu}$'s\footfullcite[These are
linear combinations of $\BesselJ_{\pm\ii \nu}$'s which are orthogonal;
see][]{Dunster1990}.

\item For the phantom model $\rbr{-, +}$:
$\widehat{H}_\text{eff}$ is bounded \alert{above}; generalised eigenfunctions 
for negative spectrum / `bound states' are the Besselian $\BesselJ_{\nu}$'s.
\end{itemize}
...
Hamiltonian is desirable, which is typically an unbounded operator in the
Hilbert space $\mscrF$ endowed with the Schr\"odinger inner product
$\rbr{\cdot,\cdot}_\text{S}$.

\item Mathematically\footfullcite{hall2013}, an \emph{unbounded} operator $H$ is
characterised not only by its action on a vector, but also by its domain
$\rfun{\mathrm{Dom}}{H} \subsetneq \mscrF$.

\item In addition to symmetricity $\rbr{\phi_1, H^\dagger \phi_2} \equiv
\rbr{H \phi_1, \phi_2}$, the self-adjointness of an unbounded operator also
requires $\rfun{\mathrm{Dom}}{H^\dagger} = \rfun{\mathrm{Dom}}{H}$.

\item For quantum systems not bounded below, not only theself-adjointness, but
...
\begin{frame}%
{The self-adjointness of Hamiltonian}%
%{General theory for unbounded operators}
\begin{itemize}
\item For the quintessence model $\rbr{+, +}$:
$\widehat{H}_\text{eff}$ is \textcolor{blue}{essentially self-adjoint} with
respect to $\cbr{\BesselK_{\ii \nu}}$.

\item For the quintessence model $\rbr{+, -}$:
%$\widehat{H}_\text{eff}$ is \textcolor{blue}{self-adjoint} with
%respect to $\cbr{\BesselF_{\ii \nu},\BesselG_{\ii \nu}}$.

\item For the phantom model $\rbr{-, +}$:
$\widehat{H}_\text{eff}$ is \alert{not symmetric} with respect to
$\cbr{\BesselJ_{\nu}}$:
\begin{align}
\rbr{\BesselJ_{\mu}, \widehat{p}^2 \BesselJ_{\nu}}_\text{S} - 
\rbr{\widehat{p}^2 \BesselJ_{\mu}, \BesselJ_{\nu}}_\text{S} =
\frac{2}{\pp}\sqrt{\mu\nu}\,\sin \frac{\pp\rbr{\mu-\nu}}{2}.
\end{align}

\begin{itemize}
\item Imposing symmetricity suggests $\nu = 2n+a$, $n \in \BbbZ_\ge$,
$a \in [0,2)$.

\item $a$ parametrises different self-adjoint extensions; in terms of the full
wave-function,
\begin{align}
\rfun{\Psi}{\beta,\chi} = \ee^{\ii\pp a} \rfun{\Psi}{\beta+,\chi}
\end{align}

\item Imposing (anti-)periodic boundary condition would fix $a = 1$ or $0$.
...
% $\rfun{\Psi}{\beta,\chi} = \ee^{-\ii k_\beta\beta} 
...
\begin{itemize}
\item $PT$-symmetric Liouville field\footfullcite{ANDRIANOV2010,Andrianov2016}: 
