...
\begin{align}
\begin{aligned}
\rfun{s_{++}}{\gamma} &\coloneqq \rfun{\sech}{\gamma},
&\qquad
\rfun{s_{+-}}{\gamma} &\coloneqq \rfun{\csch}{\gamma},\\
\rfun{s_{-+}}{\gamma} &\coloneqq \rfun{\sec}{\gamma},
&\qquad
\rfun{s_{--}}{\gamma} &\coloneqq \rfun{\ii\csc}{\gamma}.\\
\end{aligned}
\end{align}
\begin{itemize}
...
+ \rfun{\widehat{V}_{\text{eff}}}{x}} \psi,\quad
V_{\text{eff}} \coloneqq
...
\footfullcite{Morse1929,Gitman2012}.
\end{itemize}

\item Let $\ee^{2\widetilde{x}} =
...
{Results for the Liouville potential\footfullcite[sec.~8.5]{Gitman2012}}
\begin{itemize}
\item For $V_{\text{eff}} > 0$: $\widehat{H}_\text{eff}$ is
\textcolor{blue}{essentially self-adjoint}%
\footfullcite[See also][sec.~9.9]{hall2013}.
\begin{itemize}
\item For the quintessence model $\rbr{+, +}$, nothing special.
\end{itemize}

\item For $V_{\text{eff}} < 0$: $\widehat{H}_\text{eff}$ admits
\alert{a family of self-adjoint extensions}.
\begin{itemize}
\item For the quintessence model $\rbr{+,-}$, the generalised eigenfunction is a
linear combination of $\BesselF$ and $\BesselG$, and
...
\rfun{\Psi_{-+,n}}{\beta + T_\beta,\chi}$. Imposing (anti-)periodic
boundary condition would fix $a_< = 0$ ($1$).
\end{itemize}
...
non-trivial.
\begin{itemize}
\item The degeneracy of quintessence model $\rbr{+,-}$ was removed.
\item The levels of phantom Liouville model were discretised.
\end{itemize}
...
\item Beyond single Liouville field: multiple potential / multiple potential

\item Beyond classical matter and Schrödinger inner product: $PT$ field
\end{itemize}
\end{frame}
